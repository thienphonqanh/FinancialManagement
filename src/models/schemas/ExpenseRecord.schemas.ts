...
interface ExpenseRecordType {
  ...
  transfer_to_account_id?: ObjectId // Chuyển vào tài khoản (Required nếu có chọn chuyển khoản)
...
export default class ExpenseRecord {
  ...
  constructor(expenseRecordType: ExpenseRecordType) {
    ...
    this.cost_incurred_category_id = expenseRecordType.cost_incurred_category_id || new ObjectId()
    this.debtor = expenseRecordType.debtor || ''
    this.debt_collection_date = expenseRecordType.debt_collection_date || date
    this.transfer_to_account_id = expenseRecordType.transfer_to_account_id || new ObjectId()
